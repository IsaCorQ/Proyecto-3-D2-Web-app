...
void FindLongestMatch(unsigned char *s, unsigned int buf_start, unsigned short &len, unsigned short &off, LinkedList dict[])
{
    ...
    unsigned int j = 0;
    unsigned int k = 0;
    if (current != nullptr && (current->index) >= max_offset)
    {
        len = 1;
    ...
    while (current != nullptr && (current->index) >= max_offset)
    {
        ...
        {
            if ((current->index) + j == buf_start - 1)
            {
                j = 0;
            }
            else
...
void compressData(unsigned int file_size, unsigned char *data, fstream &file_out)
{
    ...
    for (unsigned int i = 0; i < file_size; i++)
    {
        ...
        {
            while ((cur_dict_length < max_dict_length) && cur_dict_length < (i + 1))
                cur_dict_length++;
        }
    }
    if (hasOddSymbol == true)
    {
        lz77_coded.push_back((unsigned char)0x1);
    }
    else
        lz77_coded.push_back((unsigned char)0x0);
...
void uncompressData(unsigned int file_size, unsigned char *data, fstream &file_out)
{
    ...
    while (i < (file_size - 1))
    {
        if (i != 0)
        {
            lz77_decoded.push_back(myLink.next);
        }
        ...
        if (myLink.length != 0)
        {
            ...
            if (myLink.length > myLink.offset)
            {
                ...
                lz77_decoded.insert(lz77_decoded.end(), lz77_decoded.end() - myLink.offset, lz77_decoded.end() - myLink.offset + myLink.length);
            i++;
        }
        else
        {
    ...
    if (hasOddSymbol == 0x0 && file_size > 1)
    {
        lz77_decoded.push_back(myLink.next);
    }
...
/*void modeSelected(string option,string inputFile,string outputFile){
    ...
    {
        printf("Unknown parameter, use -c or -u\n");
        return;
    }
    filename_in ="../"+inputFile; // input
    filename_out = "../"+outputFile; // output
...
int main(int argc, char *argv[]) // Run ./lz77 -c input outpu
{
    max_buf_length = 15;    // 16-1;
    max_dict_length = 4095; // 4096-1
    string filename_in = "";
    ...
    if (argc == 4)
    {
        ...
        filename_in = std::string(argv[2]); // input
        filename_out = std::string(argv[3]); // output
    }
    else
    {
